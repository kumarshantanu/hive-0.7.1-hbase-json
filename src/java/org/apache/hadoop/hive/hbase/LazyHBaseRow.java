  ...
  private Object uncheckedGetField(int fieldID) {
    if (!getFieldInited()[fieldID]) {
      ...
      if (HBaseSerDe.isSpecialColumn(columnFamily)) {
        ...
        else if(columnQualifier.endsWith("]")) {
        	String newColumnName = columnQualifier.split("\\[")[0];
        	Object val = "";
        	if(getField(fieldID) instanceof LazyJsonArray) {
        		((LazyJsonArray)getField(fieldID)).init(new JSONArray());
        	}
        	if(result.containsColumn(columnFamilyBytes, newColumnName.getBytes())) {
    			String jsonString = new String(result.getValue(columnFamilyBytes, newColumnName.getBytes()));
    			ref = new ByteArrayRef();
    			ref.setData("".getBytes());
				try {
					JSONObject jso;
					jso = new JSONObject(new JSONTokener(jsonString));
					val = extractFromJson(columnQualifier, jso);
					if(val != null) {
