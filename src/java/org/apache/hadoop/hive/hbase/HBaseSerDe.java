...
public class HBaseSerDe implements SerDe {
  ...
  public static int parseColumnMapping(
	  ...
	  return parseColumnMapping(columnMapping, colFamilies, colFamiliesBytes,
			  colQualifiers, colQualifiersBytes, true);
  }
  ...
  public static int parseColumnMapping(
      ...
      List<byte []> colFamiliesBytes,
      List<String> colQualifiers,
      List<byte []> colQualifiersBytes,
      boolean stripNonHbaseParts) throws SerDeException {
    ...
    for (int i = 0; i < mapping.length; i++) {
      ...
      if (elem.equals(HBASE_KEY_COL)) {
        ...
        if(elem.indexOf("[")>0 && stripNonHbaseParts){
          elem = elem.split("\\[")[0];
        }
  ...
  private void initHBaseSerDeParameters(
    ...
    iKey = parseColumnMapping(hbaseColumnsMapping, hbaseColumnFamilies,
        hbaseColumnFamiliesBytes, hbaseColumnQualifiers, hbaseColumnQualifiersBytes, false);
